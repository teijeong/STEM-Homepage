...
tmpDB = "select HH from ViewCounter where Register >= Convert(DateTime, '" & FDATE & " 00:00:00') and Register <= Convert(DateTime, '" & EDATE & " 23:59:59')"
...
For C = 0 to 23
	sql = "select Count(*) AS TotalVisit from (" & tmpDB & ") tmpDB where HH = '" & C & "'"
	GetRs Rs, sql
...
%>
	<tr bgcolor=#ffffff onmouseover="this.style.backgroundColor='#F2F9F7'" onmouseout="this.style.backgroundColor='#FFFFFF'">
		<td><%=ReSize(C, 2)%>:00:00 - <%=ReSize(C, 2)%>:59:59</td>
		<td><%=TotalVisit%></td>
