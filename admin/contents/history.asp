...
Sql="select idx,title from historyAdmin Where bansort="&bansort&" order by listnum ASC, idx ASC"
...
function boardView(){
	...
	openWindow(920,530,'historyEdit.asp?Idx='+idx,'add','yes')
}
function boardDel(){
	if(document.goods_form.selcode.selectedIndex==-1){
	...
	if(value){
		idx=document.goods_form.selcode[document.goods_form.selcode.selectedIndex].value;
		location.href='historyDel.asp?idx='+idx;
	}
}
function goAdd(){
...
function movesend() {
	...
	document.goods_form.action="historyNumChangeOk.asp"
				...
				<tr>
					<td>
						<table cellpadding="0" cellspacing="0" class="menu" width="100%">
							<tr>
								<td style='color:#39518C;' class='menu'><img src='/admin/image/titleArrow2.gif'><b>History관리</td>
								<td align='right'>
									<select name='bansort' style='background-Color: #B0CEFF;' onchange="location.href='?bansort='+this.value;">
										<% For i=2000 To Year(Date())+1 %>
										<option value="<%=i%>" <%=selcheck(i,bansort)%>><%=i%>년 History 리스트</option>
										<% Next %>
									</select>
								</td>
							</tr>
...
<table width="100%" border="1" cellpadding="0" cellspacing="0" bordercolor='#BDBEBD' class="menu" style='border-collapse: collapse'>
	...
	<tr align="center">
		<td width='150' align="center" bgcolor="#F6F6F6">일시</td>
	...
	<tr align="center">
		...
		<td align='left' style='padding: 5px;'>
			<textarea name='content' style='width:100%; height:70px;' class='ckeditor'></textarea>
		</td>
	</tr>
</table>
</form>
