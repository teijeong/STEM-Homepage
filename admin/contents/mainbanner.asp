...
Sql="select idx,title,filenames from mainbannerAdmin Where bansort="&bansort&" order by listnum ASC, idx ASC"
...
function boardView(){
	...
	openWindow(770,240,'mainbannerEdit.asp?Idx='+idx,'add','yes')
}
function boardDel(){
	if(document.goods_form.selcode.selectedIndex==-1){
	...
	if(value){
		idx=document.goods_form.selcode[document.goods_form.selcode.selectedIndex].value;
		location.href='mainbannerDel.asp?idx='+idx;
	}
}
function goAdd(){
...
function movesend() {
	...
	document.goods_form.action="mainbannerNumChangeOk.asp"
				...
				<tr>
					<td>
						<table cellpadding="0" cellspacing="0" class="menu" width="100%">
							<tr>
								<td style='color:#39518C;' class='menu'><img src='/admin/image/titleArrow2.gif'><b>배너이미지관리</td>
								<td align='right'>
									<select name='bansort' style='background-Color: #B0CEFF;' onchange="location.href='?bansort='+this.value;">
										<option value='1' <%=SelCheck(1,bansort)%>>메인 전광판배너 리스트</option>
									</select>
								</td>
							</tr>
						</table>
		...
		<col width='195'></col>
			...
			<tr>
				<td colspan='2' align='right'>
					...
					메인 전광판배너 이미지 최적사이즈[730px * 450px], 상위 5개까지 해당코너에 나타납니다.<br>
				</td>
			</tr>
		</table>
	</td></tr>
...
<table width="100%" border="1" cellpadding="0" cellspacing="0" bordercolor='#BDBEBD' class="menu" style='border-collapse: collapse'>
	...
	<tr align="center">
		<td width='150' align="center" bgcolor="#F6F6F6">타이틀</td>
		<td align='left' style='padding: 5px;'><input type='text' name='title' style='width:100%;' class='input' maxlength='50'></td>
	</tr>
	<tr align="center">
		<td align="center" bgcolor="#F6F6F6">링크주소</td>
		<td align='left' style='padding: 5px;'><input type='text' name='linkurl' style='width:100%;' class='input' maxlength='100'></td>
	</tr>
	<tr align="center">
		<td align="center" bgcolor="#F6F6F6">이미지업로드</td>
		<td align='left' style='padding: 5px;'><input type='file' name='files' style='width:80%;' class='input'></td>
	</tr>
</table>
</form>
